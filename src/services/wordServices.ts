...
export class WordServices{
    ...
    exportWordFileFromArray(file:File ,plt:Platform,socialSharing:SocialSharing,wordDataList:Array<WordData>,filename:string):Promise<any>
    {
        return new Promise(function(resolve, reject)
        {
            let allDataArray=[];
            ...
            if (plt.is('ios')) {
                // This will only print when on iOS
                file.writeFile(file.tempDirectory,filename ,csvContent+"",{replace: true}).then(value=>{
                    console.log("file write succ"+value.nativeURL);
                    socialSharing.share(null,null,null,value.nativeURL).then(data=>{
                        resolve();
                    }).catch(err=>{
                        resolve();
                    });
