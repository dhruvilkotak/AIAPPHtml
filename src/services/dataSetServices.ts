...
export class DataSetService {
  ...
  getDataSetList(file: File, wordType: number): Promise<any> {
    return new Promise(function(resolve, reject) {
      ...
      var error = "";
      var filename = "";
      if (wordType == 0) filename = "datasetDetails";
      else filename = "mathDatasetDetails";
  ...
  checkDataSetExist(dataSetDetails: Array<Dataset>, DatasetObject: Dataset) {
    ...
    dataSetDetails.forEach(datasetObj => {
      if (datasetObj.datasetName == DatasetObject.datasetName) exist = true;
    });
    return exist;
  }
  ...
  addDataSettoFile(
    file: File,
    dataSetObj: Dataset,
    dataServiceObject: DataSetService,
    wordType: number
  ): Promise<any> {
    return new Promise(function(resolve, reject) {
      var fileData: any;
      ...
      dataServiceObject
        ...
        .then(dataSetDetails => {
          ...
          if (dataSetDetails.length > 0) {
            var exist = dataServiceObject.checkDataSetExist(
              dataSetDetails,
              dataSetObj
            );
            ...
            if (exist) {
              ...
              console.log(
                "Number of Dataset added size:" + dataSetDetails.length
              );
              ...
              file
                .writeFile(
                  file.dataDirectory,
                  "dataSetDetails",
                  JSON.stringify({ dataSetDetails: dataSetDetails }),
                  { replace: true }
                )
                .then(_ => {
