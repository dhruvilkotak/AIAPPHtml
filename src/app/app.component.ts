    ...
    this.events.subscribe("user:loggedin", () => {
      ...
      this.storage.get("userDetails").then(val => {
        ...
        this.storage.get("organizationDetails").then(val => {
          ...
          console.log(
            "user logged:in:" + this.organizationDetails.organizationDetailsUID
          );
        });
  ...
  confirmTimeOut() {
    ...
    var intervalVar = setInterval(
      function() {
        this.counter--;
        alert.setMessage(
          `Do you want to stay logged in? Your session is about to expire in <strong style="color:blue">` +
            this.counter +
            `<\strong>.`
        );
        if (this.counter == 0) {
    ...
    let alert = this.alertCtrl.create({
      ...
      message:
        "Your session is about to expire in . Do you want to stay logged in ?",
      enableBackdropDismiss: false,
      buttons: [
        {
          text: "Yes",
