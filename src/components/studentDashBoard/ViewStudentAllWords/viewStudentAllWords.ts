...
export class ViewStudentAllWords {
  ...
  filterItems() {
    this.newLearnedWords = this.allData_newLearnedWords.filter(
      newKnownUnknownObject => {
        return (
          newKnownUnknownObject.wordData.wordText
            .toLowerCase()
  ...
  addWordToStudentToFile(wordDataObj: WordData, studentObject: Student) {
    ...
    allData = allData
      .concat(
        studentObject.studentWordDetailsArray[this.wordType].unKnownArrayList
      )
      .concat(
        studentObject.studentWordDetailsArray[this.wordType]
          .knownUnknownArrayList
      )
      .concat(
        studentObject.studentWordDetailsArray[this.wordType].knwonArrayList
      );
    for (let methodObj of studentObject.studentWordDetailsArray[this.wordType]
      .methodArray) {
    ...
    if (!wordServiceObj.checkWordExist(allData, wordDataObj)) {
      studentObject.studentWordDetailsArray[
        this.wordType
