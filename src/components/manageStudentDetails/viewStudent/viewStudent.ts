...
export class ViewStudent {
  ...
  filterItems() {
    this.studentDetailsArray = this.allData.filter(student => {
      return (
        student.firstName.toLowerCase().indexOf(this.searchTerm.toLowerCase()) >
          -1 ||
        student.lastName.toLowerCase().indexOf(this.searchTerm.toLowerCase()) >
          -1 ||
        student.studentId.toLowerCase().indexOf(this.searchTerm.toLowerCase()) >
          -1
      );
    });
  }
    ...
    let alert = this.alertCtrl.create({
      ...
      buttons: [
        ...
        },
        {
          text: "yes",
          handler: () => {
            this.studentServicesObject.removeStudentFromArray(
              this.allData,
              studentObj
            );
            this.filterItems();
