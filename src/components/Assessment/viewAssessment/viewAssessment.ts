...
export class ViewAssessmentTest {
  ...
  getAssessmentObject(wordData: WordData) {
    for (let obj of this.assessmentWordDataArray) {
      if (obj.wordData.wordId == wordData.wordId) {
        console.log(
          "same : " + obj.wordData.wordText + " s:" + wordData.wordText
        );
        return obj;
      }
    }
  ...
  updateAssessmentObjectToArray(
    viewAssessmentWordObject: ViewAssessmentWordObjects
  ) {
    var i: number = 0;
    if (this.assessmentWordDataArray == null) this.assessmentWordDataArray = [];
    for (let obj of this.assessmentWordDataArray) {
      if (
        obj.assessmentWordObjectId ==
  ...
  saveToKnownUnknown() {
    ...
    for (let obj of this.studentObject.studentWordDetailsArray[this.wordType]
      .studentDatasetRecordList[this.studentDataSetRecordIndex]
      .assessmentWordDataArray) {
      if (!obj.wordAdded) {
        anyChanges = true;
        //loginc
        if (obj.totalKnownTime >= this.knownsTime) {
          ...
          this.studentObject.studentWordDetailsArray[
            ...
            this.studentDataSetRecordIndex
          ].knwonArrayList.push(obj.wordData);
        } else {
          ...
          this.studentObject.studentWordDetailsArray[
            ...
            this.studentDataSetRecordIndex
          ].unKnownArrayList.push(obj.wordData);
        }
        obj.wordAdded = true;
      }
    }
    this.assessmentWordDataArray = this.studentObject.studentWordDetailsArray[
      this.wordType
    ].studentDatasetRecordList[
      this.studentDataSetRecordIndex
    ].assessmentWordDataArray;
  ...
  chekEnableDisable() {
    ...
    var previBool: boolean = this.studentObject.studentWordDetailsArray[
      this.wordType
    ].studentDatasetRecordList[this.studentDataSetRecordIndex].isConvertedAll;
    if (previBool) {
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.studentObject.studentWordDetailsArray[
              ...
              this.studentDataSetRecordIndex
            ].knwonArrayList.push(wordDataObj);
  ...
  unKnownConfirm(wordDataObj: WordData) {
    let alert = this.alertCtrl.create({
      title: "unKnown word",
      message:
        "Do you want to set as unKnown word" + wordDataObj.wordText + "?",
      buttons: [
        {
          text: "Cancel",
          role: "cancel",
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.studentObject.studentWordDetailsArray[
              ...
              this.studentDataSetRecordIndex
            ].unKnownArrayList.push(wordDataObj);
  ...
  showKnownUnKnownWords() {
    ...
    for (let assessmentwordObject of this.assessmentWordDataArray) {
      ...
      if (
        assessmentwordObject.wordAdded != null &&
        assessmentwordObject.wordAdded
      ) {
        if (assessmentwordObject.wordType == "Known") this.totalKnowns++;
        else this.totalUnKnowns++;
      } else {
